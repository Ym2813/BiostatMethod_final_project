...
library(leaps)
Cp_b = leaps(x = model.matrix(trans_fit)[,-1],
      y = cdi_trans$crm_1000_sqr,
      nbest = 2,
      method = "Cp")

mod_b = Cp_b$Cp[which(Cp_b$Cp <= Cp_b$size)]
```
...
**forward**
```{r, include=FALSE}
Cp_f = leaps(x = model.matrix(fit_forward)[,-1],
      y = cdi_trans$crm_1000_sqr,
      nbest = 2,
      method = "Cp")

mod_f = Cp_f$Cp[which(Cp_f$Cp <= Cp_f$size)]
```
